// ─────────────────────────────────────────────
//  Provider SVG icon paths (Simple Icons)
// ─────────────────────────────────────────────
const ICON_PATHS = {
...
const COLORS_LIGHT = {
  OpenAI:    { line: '#059669', bg: 'rgba(5,150,105,0.85)',  dim: 'rgba(5,150,105,0.12)'  },
  Anthropic: { line: '#ea580c', bg: 'rgba(234,88,12,0.85)',  dim: 'rgba(234,88,12,0.12)'  },
  Google:    { line: '#2563eb', bg: 'rgba(37,99,235,0.85)',  dim: 'rgba(37,99,235,0.12)'  },
};
function getC(prov) {
  return (document.documentElement.dataset.theme === 'light' ? COLORS_LIGHT : COLORS_DARK)[prov];
}

function chartDefaults() {
  const light = document.documentElement.dataset.theme === 'light';
  return {
    tick:        light ? '#5a6280' : '#7c84a3',
    grid:        light ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.04)',
    tooltipBg:   light ? 'rgba(250,250,255,0.98)' : 'rgba(10,12,24,0.97)',
    tooltipText: light ? '#1a1b2e' : '#e8eaf6',
    tooltipMuted:light ? '#5a6280' : '#7c84a3',
  };
}
...
let timelineChart, compareChart, minimapChart;
let rangeSetupDone = false;

// ─────────────────────────────────────────────
//  Date range (full span + 2-month padding)
// ─────────────────────────────────────────────
const _allDates = MODELS.map(m => new Date(m.date));
const MAP_MIN = new Date(Math.min(..._allDates).valueOf());
const MAP_MAX = new Date(Math.max(..._allDates).valueOf());
MAP_MIN.setMonth(MAP_MIN.getMonth() - 2);
MAP_MAX.setMonth(MAP_MAX.getMonth() + 3);

let rangeStart = 0;  // 0.0–1.0 fraction of MAP_MIN→MAP_MAX
let rangeEnd   = 1;

function fracToDate(f) {
  return new Date(MAP_MIN.getTime() + f * (MAP_MAX - MAP_MIN));
}
function dateToFrac(d) {
  return (d - MAP_MIN) / (MAP_MAX - MAP_MIN);
}
...
//  Metric metadata
// ─────────────────────────────────────────────
const metricLabel = { context: 'Context Window', output: 'Max Output Tokens', reasoning: 'Reasoning Tokens', breakdown: 'I/O Breakdown' };
...
function providerClass(p) { return p.toLowerCase(); }
...
function buildStats() {
  ...
  const stats  = [
    ...
    { num: Math.round(maxCtx / minCtx).toLocaleString() + '×', label: 'Growth since 2020' },
  ];
...
function buildTimeline() {
  ...
  document.getElementById('timelineTitle').textContent = metricLabel[timelineMetric] + ' over time (log scale)';
    ...
    options: {
      ...
      plugins: {
        legend: {
          ...
          labels: { color: cd.tick, usePointStyle: true, pointStyle: 'circle', padding: 20, font: { size: 13 } },
        },
        tooltip: {
          backgroundColor: cd.tooltipBg,
          borderColor: 'rgba(129,140,248,0.2)', borderWidth: 1,
          padding: 14,
          titleFont: { size: 14, weight: 'bold' }, titleColor: cd.tooltipText,
          bodyFont: { size: 12 }, bodyColor: cd.tooltipMuted,
          callbacks: {
            title: items => items[0].raw.model.name,
            label: item => {
              const m  = item.raw.model;
              const dt = new Date(m.date).toLocaleDateString('en-US', { year: 'numeric', month: 'short' });
              const lines = [
                `  Provider : ${m.provider}`,
                `  Released : ${dt}`,
      ...
      scales: {
        x: {
          type: 'time',
          min: fracToDate(rangeStart),
          max: fracToDate(rangeEnd),
          time: { unit: 'month', tooltipFormat: 'MMM yyyy', displayFormats: { month: 'MMM yy', year: 'yyyy' } },
          grid: { color: cd.grid },
          ticks: { color: cd.tick, font: { size: 11 }, maxRotation: 0 },
        },
        y: {
          type: 'logarithmic',
          grid: { color: cd.grid },
          ticks: { color: cd.tick, font: { size: 11 }, callback: v => fmtTokens(v), includeBounds: false },
        },
      },
    },
  });
...
// ─────────────────────────────────────────────
//  Minimap (overview chart)
// ─────────────────────────────────────────────
function buildMinimap() {
  const canvas = document.getElementById('minimapChart');
  const ctx    = canvas.getContext('2d');
  if (minimapChart) minimapChart.destroy();
  ...
  const datasets = providers.map(prov => {
    ...
    const pts = models
      .filter(m => m.provider === prov)
      .sort((a, b) => new Date(a.date) - new Date(b.date))
      .map(m => ({ x: m.date, y: m[key] }));
    return {
      data: pts,
      borderColor: c.line, backgroundColor: c.bg,
      pointRadius: 2.5, pointHoverRadius: 2.5,
...
// ─────────────────────────────────────────────
//  Year tick labels below minimap
// ─────────────────────────────────────────────
function buildYearTicks() {
  const el   = document.getElementById('minimapTicks');
  const span = MAP_MAX - MAP_MIN;
  const html = [];
  const startY = MAP_MIN.getFullYear();
  const endY   = MAP_MAX.getFullYear() + 1;
  for (let y = startY; y <= endY; y++) {
    const d   = new Date(y, 0, 1);
    const pct = Math.max(0, Math.min(100, (d - MAP_MIN) / span * 100));
    html.push(`<span class="ytick" style="left:${pct.toFixed(2)}%">${y}</span>`);
  }
  el.innerHTML = html.join('');
}

// ─────────────────────────────────────────────
//  Range selector state & UI update
// ─────────────────────────────────────────────
function updateRangeUI() {
  const leftMask   = document.getElementById('leftMask');
  const rangeWin   = document.getElementById('rangeWindow');
  const selLabel   = document.getElementById('selRangeLabel');
  if (!leftMask) return;

  leftMask.style.width  = (rangeStart * 100).toFixed(3) + '%';
  rangeWin.style.width  = ((rangeEnd - rangeStart) * 100).toFixed(3) + '%';

  if (rangeStart === 0 && rangeEnd === 1) {
    selLabel.textContent = 'All time';
  } else {
    selLabel.textContent = formatShortDate(fracToDate(rangeStart)) + ' → ' + formatShortDate(fracToDate(rangeEnd));
  }

  // Sync main chart axis
  if (timelineChart) {
    timelineChart.options.scales.x.min = fracToDate(rangeStart);
    timelineChart.options.scales.x.max = fracToDate(rangeEnd);
...
function setRange(s, e) {
  ...
  rangeStart = Math.max(0, Math.min(s, 1 - MIN_WIDTH));
  rangeEnd   = Math.min(1, Math.max(e, rangeStart + MIN_WIDTH));
  updateRangeUI();
}

function resetRange() { setRange(0, 1); }

// ─────────────────────────────────────────────
//  Range selector drag interaction (set up once)
// ─────────────────────────────────────────────
function setupRangeSelector() {
  ...
  let dragging      = null;   // 'left' | 'right' | 'window' | 'lmask' | 'rmask'
  let dragStartFrac = 0;
  let dragStartRS   = 0;
  let dragStartRE   = 0;
  ...
  function onMove(e) {
    ...
    e.preventDefault();
    const f = getFrac(e);
    if (dragging === 'left')  { setRange(f, rangeEnd);   }
    if (dragging === 'right') { setRange(rangeStart, f); }
    if (dragging === 'window') {
      const delta = f - dragStartFrac;
      const w     = dragStartRE - dragStartRS;
      const s     = Math.max(0, Math.min(1 - w, dragStartRS + delta));
      setRange(s, s + w);
    }
    if (dragging === 'lmask') { setRange(f, rangeEnd); }
    if (dragging === 'rmask') { setRange(rangeStart, f); }
  }

  function onUp() { dragging = null; }

  // Mouse
  leftHandle.addEventListener('mousedown',  e => { dragging = 'left';  e.stopPropagation(); e.preventDefault(); });
  rightHandle.addEventListener('mousedown', e => { dragging = 'right'; e.stopPropagation(); e.preventDefault(); });
  rangeWin.addEventListener('mousedown', e => {
    if (e.target === leftHandle || e.target === rightHandle) return;
    dragging = 'window'; dragStartFrac = getFrac(e); dragStartRS = rangeStart; dragStartRE = rangeEnd;
    e.preventDefault();
  });
  leftMask.addEventListener('mousedown',  e => { dragging = 'lmask'; });
  rightMask.addEventListener('mousedown', e => { dragging = 'rmask'; });

  // Touch
  leftHandle.addEventListener('touchstart',  e => { dragging = 'left';  e.stopPropagation(); }, { passive: false });
  rightHandle.addEventListener('touchstart', e => { dragging = 'right'; e.stopPropagation(); }, { passive: false });
  rangeWin.addEventListener('touchstart', e => {
    if (e.target === leftHandle || e.target === rightHandle) return;
    dragging = 'window'; dragStartFrac = getFrac(e); dragStartRS = rangeStart; dragStartRE = rangeEnd;
  }, { passive: false });
  leftMask.addEventListener('touchstart',  () => { dragging = 'lmask'; }, { passive: true });
  rightMask.addEventListener('touchstart', () => { dragging = 'rmask'; }, { passive: true });
...
function buildCompareSingle(key) {
  ...
  const bgs     = models.map(m => getC(m.provider).bg);
  const borders = models.map(m => getC(m.provider).line);
  ...
  compareChart = new Chart(ctx, {
    type: 'bar',
    data: { labels: models.map(m => m.name), datasets: [{ label: metricLabel[compareMetric], data: models.map(m => m[key]), backgroundColor: bgs, borderColor: borders, borderWidth: 1.5, borderRadius: 6 }] },
    options: {
      indexAxis: 'y', responsive: true, maintainAspectRatio: false,
      plugins: {
        legend: { display: false },
        tooltip: {
          backgroundColor: cd.tooltipBg, borderColor: 'rgba(129,140,248,0.2)', borderWidth: 1,
          padding: 14, titleFont: { size: 13, weight: 'bold' }, titleColor: cd.tooltipText,
          bodyFont: { size: 12 }, bodyColor: cd.tooltipMuted,
          callbacks: {
            title: items => items[0].label,
            label: item => {
              const m = models[item.dataIndex];
              const ratio = m.output && m.context > m.output ? ` (${(m.output / m.context * 100).toFixed(1)}% of ctx)` : '';
              return [
      ...
      scales: {
        x: { type: 'logarithmic', grid: { color: cd.grid }, ticks: { color: cd.tick, font: { size: 11 }, callback: v => fmtTokens(v) } },
        y: { grid: { display: false }, ticks: { color: cd.tooltipText, font: { size: 11 } } },
      },
    },
  });
  document.getElementById('compareChart').parentElement.style.height = Math.max(400, models.length * 32 + 60) + 'px';
}

// ─────────────────────────────────────────────
//  Compare – I/O breakdown stacked
// ─────────────────────────────────────────────
function buildCompareBreakdown() {
  ...
  compareChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        { label: 'Input Capacity', data: models.map(m => Math.max(0, m.context - m.output)), backgroundColor: models.map(m => getC(m.provider).line + 'aa'), borderWidth: 0 },
        { label: 'Max Output',     data: models.map(m => Math.max(0, m.output - (m.reasoning || 0))), backgroundColor: 'rgba(249,115,22,0.78)', borderWidth: 0 },
        { label: 'Reasoning',      data: models.map(m => m.reasoning || 0), backgroundColor: 'rgba(196,132,252,0.82)', borderWidth: 0 },
      ],
    },
    options: {
      indexAxis: 'y', responsive: true, maintainAspectRatio: false,
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: item => {
              const m = models[item.dataIndex];
              const r = (m.output / m.context * 100).toFixed(1);
              return [
                `  Context Window : ${fmtTokens(m.context)}`,
                `  Input Capacity : ${fmtTokens(Math.max(0, m.context - m.output))}`,
                `  Max Output     : ${fmtTokens(m.output)} (${r}% of ctx)`,
                ...(m.reasoning > 0 ? [`  Reasoning      : ${fmtTokens(m.reasoning)}`] : []),
                `  Provider       : ${m.provider}`,
              ];
            },
      ...
      scales: {
        x: { stacked: true, type: 'logarithmic', grid: { color: cd.grid }, ticks: { color: cd.tick, font: { size: 11 }, callback: v => fmtTokens(v) } },
        y: { stacked: true, grid: { display: false }, ticks: { color: cd.tooltipText, font: { size: 11 } } },
      },
    },
  });
  document.getElementById('compareChart').parentElement.style.height = Math.max(400, models.length * 32 + 60) + 'px';
...
function buildCards() {
  ...
  document.getElementById('modelGrid').innerHTML = filteredModels()
    ...
    .map(m => {
      ...
      const outputW = Math.max(0.6, m.output / maxCtx * 100).toFixed(2);
...
function render() {
  ...
  if (activeView === 'timeline') {
    ...
    buildMinimap();
    buildYearTicks();
    if (!rangeSetupDone) { setupRangeSelector(); rangeSetupDone = true; }
  }
...
// ─────────────────────────────────────────────
function initTheme() {
  const saved    = localStorage.getItem('llm-theme');
...
document.querySelectorAll('#timelineMetrics .metric-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (activeView === 'timeline') { buildTimeline(); buildMinimap(); }
  });
});
...
// Window resize: rebuild active chart
let _resizeTimer;
window.addEventListener('resize', () => {
  clearTimeout(_resizeTimer);
  _resizeTimer = setTimeout(() => { if (activeView === 'timeline') { buildTimeline(); buildMinimap(); } }, 220);
});
